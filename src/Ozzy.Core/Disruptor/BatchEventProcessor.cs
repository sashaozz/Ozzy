...
namespace Disruptor
{
    ...
    internal sealed class BatchEventProcessor<T> : IEventProcessor where T : class
    {
        ...
        public BatchEventProcessor(RingBuffer<T> ringBuffer, ISequenceBarrier sequenceBarrier, IEventHandler<T> eventHandler, long startSequence)
        {
            _sequence = new Sequence(startSequence);
