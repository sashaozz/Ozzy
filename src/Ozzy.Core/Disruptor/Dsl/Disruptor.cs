...
using Volatile = Ozzy.Core.Disruptor.Volatile;
...
namespace Disruptor.Dsl
{
    ...
    public class Disruptor<T> where T : class
    {
        ...
        /// <param name="startSequence"></param>
        /// <returns>a <see cref="EventHandlerGroup{T}"/> that can be used to chain dependencies.</returns>
        public EventHandlerGroup<T> HandleEventsWith(long startSequence = Sequencer.InitialCursorValue, params IEventHandler<T>[] handlers)
        {
            return CreateEventProcessors(new IEventProcessor[0], handlers, startSequence);
        }
