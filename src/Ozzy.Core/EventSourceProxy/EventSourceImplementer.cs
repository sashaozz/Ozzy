...
#endif
{
    ...
    public static class EventSourceImplementer
    {
        ...
        /// </summary>
        private static ConcurrentDictionary<Type, TypeImplementer> _eventSources = new ConcurrentDictionary<Type, TypeImplementer>();
        #endregion
        ...
        public static T GetEventSourceAs<T>() where T : class
        {
            ...
            if (!typeof(T).GetTypeInfo().IsInterface && !typeof(T).GetTypeInfo().IsSubclassOf(typeof(EventSource)))
        ...
        /// <returns>An EventSource that is compatible with the given type.</returns>        
        public static T GetLog<TLog,T>()
        {
            return (T)GetEventSourceFactory(typeof(T)).EventSourceFactory(typeof(TLog).FullName);
        }

        public static T GetLog<T>()
        {
            return (T)GetEventSourceFactory(typeof(T)).EventSourceFactory(null);
        }

        public static EventSource GetEventSource(Type type)
        {
            return GetEventSourceFactory(type).EventSource;
        }

        private static TypeImplementer GetEventSourceFactory(Type type)
        {
            lock (_eventSources)
            {
                return _eventSources.GetOrAdd(type, t => new TypeImplementer(t));
            }
        }
        ...
        public static EventKeywords GetKeywordValue<T>(string methodName) where T : class
        {
            ...
            var keywordType = logType.GetTypeInfo().GetNestedType("Keywords");
            if (keywordType == null)
            ...
            var keyword = keywordType.GetTypeInfo().GetField(methodName);
            if (keyword == null)
