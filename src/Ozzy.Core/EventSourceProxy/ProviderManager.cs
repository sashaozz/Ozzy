...
#endif
{
	...
	class ProviderManager
	{
		...
		internal static void RegisterProvider(Type logType, Type providerType, object provider)
		{
			...
			if (!providerType.GetTypeInfo().IsInstanceOfType(provider))
		...
		private static object GetProvider(Type logType, Type providerType, Type attributeType, Func<object> defaultConstructor)
		{
			...
			return _providers.GetOrAdd(
				...
				_ =>
				{
					...
					{
						var providerAttribute = (TraceProviderAttribute)logType.GetTypeInfo().GetCustomAttributes(attributeType, true).FirstOrDefault();
						if (providerAttribute != null)
							return providerAttribute.ProviderType.GetTypeInfo().GetConstructor(Type.EmptyTypes).Invoke(null);
					}
