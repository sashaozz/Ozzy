	...
	static class ProxyHelper
	{
			...
			FieldBuilder serializationProviderField)
		{
			...
			if (context.EventLevel != null)
			{
				...
				il.Emit(OpCodes.Newobj, typeof(Dictionary<string, string>).GetTypeInfo().GetConstructor(Type.EmptyTypes));
				...
				il.Emit(OpCodes.Callvirt, typeof(TraceSerializationProvider).GetTypeInfo().GetMethod("ProvideSerialization", BindingFlags.Instance | BindingFlags.Public));
			}
			else
			...
			FieldBuilder serializationProviderField)
		{
			...
			if (context.EventLevel != null)
			{
				...
				if (sourceType.IsGenericParameter || sourceType.GetTypeInfo().IsValueType)
				...
				mIL.Emit(OpCodes.Callvirt, typeof(TraceSerializationProvider).GetTypeInfo().GetMethod("ProvideSerialization", BindingFlags.Instance | BindingFlags.Public));
			}
			else
			{
		...
		internal static bool ParametersMatch(MethodInfo m, Type[] targetTypes)
		{
			...
			for (int i = 0; i < sourceTypes.Length; i++)
			{
				...
				if (sourceType.GetTypeInfo().IsGenericType &&
					targetType.GetTypeInfo().IsGenericType &&
		...
		internal static List<MethodInfo> DiscoverMethods(Type type)
		{
			...
			for (; type != null && type != typeof(object) && type != typeof(EventSource); type = type.GetTypeInfo().BaseType)
