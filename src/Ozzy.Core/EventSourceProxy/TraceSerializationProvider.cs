	...
	public abstract class TraceSerializationProvider
	{
		...
		public virtual EventLevel? GetEventLevelForContext(TraceSerializationContext context)
		{
			...
			if (parameterInfo != null)
			{
				...
				attribute = parameterInfo.ParameterType.GetTypeInfo().GetCustomAttribute<TraceSerializationAttribute>();
				if (attribute != null)
					return attribute.Level;
			}
			...
			attribute = context.MethodInfo.DeclaringType.GetTypeInfo().GetCustomAttribute<TraceSerializationAttribute>();
			if (attribute != null)
				return attribute.Level;
