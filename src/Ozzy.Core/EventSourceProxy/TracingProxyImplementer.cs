...
#endif
{
	...
	class TracingProxyImplementer
	{
		...
		private MethodInfo ImplementProxy(EventSource log)
		{
			...
			if (_executeType.GetTypeInfo().IsInterface)
            ...
            var disposibleMethods = ProxyHelper.DiscoverMethods(typeof(IDisposable));
            foreach (MethodInfo interfaceMethod in disposibleMethods.Where(m => !m.IsFinal))
                EmitMethodImpl(interfaceMethod);
			...
			t.GetTypeInfo().GetField(_logField.Name, BindingFlags.Static | BindingFlags.NonPublic).SetValue(null, log);
			t.GetTypeInfo().GetField(_serializerField.Name, BindingFlags.Static | BindingFlags.NonPublic).SetValue(null, _serializationProvider);
			...
			return t.GetTypeInfo().GetMethod(createMethod.Name, _createParameterTypes);
		}
		...
		private MethodInfo DiscoverMethod(Type type, string methodName, string suffix, Type[] parameterTypes)
		{
			...
			var methods = type.GetTypeInfo().GetMethods().Where(m => ProxyHelper.ParametersMatch(m, parameterTypes));
			...
			if (method == null && type.GetTypeInfo().IsInterface)
			{
				foreach (Type baseInterface in type.GetTypeInfo().GetInterfaces())
				{
