...
namespace EventSourceProxy
{
    ...
    class TypeImplementer
    {
        ...
        /// </summary>
        private static MethodInfo _writeEvent = typeof(EventSource).GetTypeInfo()
            .GetMethods(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.InvokeMethod)
            .Do(mi => GetMethodInfoWithParams("WriteEvent", mi, new[] { typeof(int), typeof(object[]) }));

        /// <summary>
        /// The WriteEvent method for EventSource.
        /// </summary>
        private static MethodInfo _writeEventNoParams = typeof(EventSource).GetTypeInfo()
            .GetMethods(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.InvokeMethod)
            .Do(mi => GetMethodInfoWithParams("WriteEvent", mi, new[] { typeof(int) }));
        ...
        public TypeImplementer(Type interfaceType, TraceParameterProvider parameterProvider = null)
        {
            ...
            if (_contextProvider != null && !_interfaceType.GetTypeInfo().IsInterface)
        ...
        private void ImplementType()
        {
            ...
            t.GetTypeInfo().GetField(_invocationContextsField.Name, BindingFlags.Static | BindingFlags.NonPublic).SetValue(null, _invocationContexts.ToArray());
        ...
        private static string FoldMethodName(MethodInfo methodInfo)
        {
            ...
            if (methodInfo.DeclaringType.GetTypeInfo().GetMethods().Any(m => String.Compare(m.Name, trimmed, StringComparison.OrdinalIgnoreCase) == 0))
                return trimmed;
        ...
        private MethodBuilder EmitMethodImpl(InvocationContext invocationContext, ref int eventId, EventKeywords autoKeyword)
        {
            ...
            if (interfaceMethod.IsAbstract || !interfaceMethod.DeclaringType.GetTypeInfo().IsSubclassOf(typeof(EventSource)))
            {
        ...
        private MethodBuilder EmitMethodComplementImpl(InvocationContext invocationContext, string suffix, Type parameterType, string parameterName, MethodInfo beginMethod, ref int eventId, EventKeywords autoKeyword, MethodBuilder faultedMethod)
        {
            ...
            if (interfaceMethod.DeclaringType.GetTypeInfo().GetMethod(methodName, parameterTypes) != null)
                return null;
        ...
        private static void EmitTaskCompletion(MethodBuilder methodBuilder, Type parameterType, MethodBuilder faultedMethod)
        {
            ...
            if (parameterType != typeof(Task) && !parameterType.GetTypeInfo().IsSubclassOf(typeof(Task)))
                return;
            ...
            mIL.Emit(OpCodes.Call, typeof(Task).GetTypeInfo().GetProperty("IsCompleted").GetGetMethod());
            ...
            var callbackMethod = methodBuilder.IsGenericMethod ? methodBuilder.MakeGenericMethod(parameterType.GetTypeInfo().GetGenericArguments()) : methodBuilder;
            mIL.Emit(OpCodes.Ldftn, callbackMethod);
            mIL.Emit(OpCodes.Newobj, actionType.GetTypeInfo().GetConstructor(new Type[] { typeof(object), typeof(IntPtr) }));
            mIL.Emit(OpCodes.Ldc_I4, (int)TaskContinuationOptions.ExecuteSynchronously);
            var continuation = parameterType.GetTypeInfo().GetMethod("ContinueWith", new Type[] { actionType, typeof(TaskContinuationOptions) });
            ...
            if (faultedMethod != null)
            {
                ...
                mIL.Emit(OpCodes.Ldarg_1);
                mIL.Emit(OpCodes.Call, typeof(Task).GetTypeInfo().GetProperty("IsFaulted").GetGetMethod());
                ...
                mIL.Emit(OpCodes.Call, typeof(Task).GetTypeInfo().GetProperty("Exception").GetGetMethod());
        ...
        private void EmitCallWriteEvent(InvocationContext invocationContext, MethodBuilder methodBuilder, EventAttribute eventAttribute, List<ParameterMapping> parameterMapping)
        {
            ...
            for (int i = 0; i < parameterMapping.Count; i++)
            {
                ...
                if (parameter.HasSource)
                {
                    ...
                    if (parameter.CleanTargetType.GetTypeInfo().IsValueType)
                    {
        ...
        private void EmitEnumImplementation(Type enumSourceType, string className, Type codeType)
        {
            ...
            foreach (FieldInfo info in enumSourceType.GetTypeInfo().GetFields(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Static | BindingFlags.DeclaredOnly))
            {
        ...
        private static Type FindNestedType(Type searchType, string className)
        {
            ...
            for (; nestedType == null && searchType != null; searchType = searchType.GetTypeInfo().BaseType)
                nestedType = searchType.GetTypeInfo().GetNestedType(className);
        ...
        private static object GetDefaultStringFormatValue(Type type)
        {
            ...
            if (type.IsByRef || type.GetTypeInfo().ContainsGenericParameters || type.GetTypeInfo().IsInterface)
                return null;
        ...
        private static MethodInfo GetMethodInfoWithParams(string name, IEnumerable<MethodInfo> methods, Type[] paramTypes)
        {
            return methods.First(mi =>
            {
                ...
                {
                    if (pars[i].ParameterType != paramTypes[i]) return false;
                }
                return true;
            });
        }

        private static ConstructorInfo GetConstructorInfoWithParams(IEnumerable<ConstructorInfo> constructors, Type[] paramTypes)
        {
            return constructors.First(mi =>
            {
                var pars = mi.GetParameters();
                if (pars.Length != paramTypes.Length) return false;
                for (int i = 0; i < paramTypes.Length; i++)
